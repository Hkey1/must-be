...
mustBe.int(1);
mustBe.integer(1);
mustBe.positive.integer(1);
mustBe.positive.finite(1);
mustBe.int.finite.positive(1);

mustBe.positive.int(1);
mustBe.positiveInt(1);
mustBe.positiveInteger(1);

const someVar = 0;

mustBe.positiveFiniteInteger(someVar);
mustBe.positiveInteger(0);
...
function addFunctions(isFunctions0){
	const isFunctions = {};
	const realNames   = {};   
	for(let name in isFunctions0){
		const fun         = isFunctions0[name];
		const realName    = (typeof(fun)==='string') ? fun : name;
		isFunctions[name] = isFunctions0[realName];
	...
	const firstFuns = {};
	const allCombos = {};
	Object.entries(isFunctions).forEach(([name,check])=>{ 
		if(!name.includes('.')){
			funs[name]   = function(fun, prev, val, valName=undefined, depth=1, first=null){
				first ||= fun;
				if(prev){
					prev(val, valName, depth+1, first);
				}
			};
			firstFuns[name] = function(fun, val, valName=undefined, depth=1, first=null){
				if(!check(val)){
					throw new MustBeError('{0} mustBe.'+name+'! Given: '+given(val), valName, depth, first || fun);
				}
			};
		} else {
			allCombos[name] = function(fun, val, valName=undefined, depth=1){
				if(!check(val)){
					throw new MustBeError('{0} mustBe.'+name+'! Given: '+given(val), valName, depth, fun);
				}
			};
		}
	});
	for(let name1 in funs){
		const binded1 = firstFuns[name1].bind(mustBe, firstFuns[name1]);
		mustBe[name1] = binded1; 
		const rn1  = realNames[name1];
		for(let name2 in funs){
			const rn2  = realNames[name2];
			if(rn1!==rn2){
				const binded2 = funs[name2].bind(binded1, funs[name2], binded1);
				binded1[name2]   = binded2;
				for(let name3 in funs){
					const rn3  = realNames[name3];
					if(rn1!==rn3 && rn2!==rn3){
						binded2[name3] = funs[name3].bind(binded2, funs[name3], binded2);
					}
				}
			}
		}
	}
	for(let name in allCombos){
		const fun    = allCombos[name];
		const names  = name.split('.');
		const binded = fun.bind(mustBe, fun);
...
mustBe.int(1);
mustBe.positive.int(1);
mustBe.positive.finite.int(1);
mustBe.positive(0);

//сделать более быстрые функции вроде notEmptyArray
//?? check(,[rules])
//сделать ограгтченное число комбинаций.
*/
